...
class RV3028C7
{
	public:
		...
		bool setDateTime(uint16_t year, uint8_t month, uint8_t dayOfMonth, DayOfWeek_t dayOfWeek, uint8_t hour, uint8_t minute, uint8_t second = 0, bool syncUnixTime = true);
		...
		bool setPeriodicCountdownTimer(uint16_t timerValue, TimerClockFrequency_t frequency, bool repeat = true);
		...
		uint32_t convertToUnixTimestamp(uint16_t year, uint8_t month, uint8_t dayOfMonth, uint8_t hour, uint8_t minute, uint8_t second);
