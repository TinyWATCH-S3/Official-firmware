...
bool RV3028C7::setUnixTimestamp(uint32_t secondsSinceEpoch, bool syncCalendar)
{
	...
	uint8_t ts[4] = {(uint8_t)secondsSinceEpoch, (uint8_t)(secondsSinceEpoch >> 8), (uint8_t)(secondsSinceEpoch >> 16), (uint8_t)(secondsSinceEpoch >> 24)};
	...
	if (syncCalendar)
	{
		...
		success = setDateTime(dateTime->tm_year + 1900, dateTime->tm_mon + 1, dateTime->tm_mday, SUN, dateTime->tm_hour, dateTime->tm_min, dateTime->tm_sec, false);
		success = synchronize();
	}
...
char *RV3028C7::getCurrentDateTime()
{
	...
	sprintf(iso8601, "20%02d-%02d-%02dT%02d:%02d:%02d", convertToDecimal(_dateTime[DATETIME_YEAR]), convertToDecimal(_dateTime[DATETIME_MONTH]), convertToDecimal(_dateTime[DATETIME_DAY_OF_MONTH]), convertToDecimal(_dateTime[DATETIME_HOUR]), convertToDecimal(_dateTime[DATETIME_MINUTE]), convertToDecimal(_dateTime[DATETIME_SECOND]));
	return iso8601;
}
...
void RV3028C7::setDateTimeFromISO8601(String iso8601) { return setDateTimeFromISO8601(iso8601.c_str()); }
...
void RV3028C7::setDateTimeFromISO8601(const char *iso8601)
{
	...
	setDateTime(
		/*year=*/2000 + buffer[0], /*month=*/buffer[1],
...
// Set thw time from
bool RV3028C7::setDateTimeFromTM(tm &timeinfo) { return setDateTime((uint16_t)timeinfo.tm_year + 1900, (uint8_t)timeinfo.tm_mon + 1, (uint8_t)timeinfo.tm_mday, (uint8_t)timeinfo.tm_wday, (uint8_t)timeinfo.tm_hour, (uint8_t)timeinfo.tm_min, (uint8_t)timeinfo.tm_sec); }

void RV3028C7::setDateTimeFromHTTPHeader(String str) { return setDateTimeFromHTTPHeader(str.c_str()); }
...
void RV3028C7::setDateTimeFromHTTPHeader(const char *str)
{
	...
	setDateTime(
		year, month, dayOfMonth, static_cast<DayOfWeek_t>(dayOfWeek),
		/*hour=*/buffer[0], /*minute=*/buffer[1], /*second=*/buffer[2]
	);
}

bool RV3028C7::setDateTime(uint16_t year, uint8_t month, uint8_t dayOfMonth, DayOfWeek_t dayOfWeek, uint8_t hour, uint8_t minute, uint8_t second, bool syncUnixTime)
{
	...
	if (syncUnixTime)
	{
		uint32_t unixTimestamp = convertToUnixTimestamp(year, month, dayOfMonth, hour, minute, second);
...
void RV3028C7::setDateTimeComponent(DateTimeComponent_t component, uint8_t value)
{
	// Updates RTC date time value to array
	readBytesFromRegisters(REG_CLOCK_SECONDS, _dateTime, DATETIME_COMPONENTS);
...
bool RV3028C7::synchronize() { return writeBytesToRegisters(REG_CLOCK_SECONDS, _dateTime, DATETIME_COMPONENTS); }

bool RV3028C7::enableClockOutput(ClockOutputFrequency_t frequency)
{
	uint8_t clkout = readByteFromEEPROM(REG_EEPROM_CLKOUT);
	if ((clkout & (1 << BP_REG_EEPROM_CLKOUT_CLKOE)) > 0 && (clkout & BM_REG_EEPROM_CLKOUT_FD) == frequency)
	{
	...
	return writeByteToRegister(REG_EEPROM_CLKOUT, clkout) && writeByteToEEPROM(REG_EEPROM_CLKOUT, clkout);
}
...
bool RV3028C7::setDateAlarm(AlarmMode_t mode, uint8_t dayOfMonth, uint8_t hour, uint8_t minute)
{
	...
	if (!writeByteToRegister(REG_CONTROL_2, (control2 & ~(1 << BP_REG_CONTROL_2_AIE))))
	{
		return false;
	}
	...
	if (!writeByteToRegister(REG_CONTROL_1, (control1 | (1 << BP_REG_CONTROL_1_WADA))))
	{
		return false;
	}
...
bool RV3028C7::setWeekdayAlarm(AlarmMode_t mode, DayOfWeek_t dayOfWeek, uint8_t hour, uint8_t minute)
{
	// Clears AIE and AF bits to 0
	uint8_t status = readByteFromRegister(REG_STATUS);
	if (!writeByteToRegister(REG_STATUS, (status & ~(1 << BP_REG_STATUS_AF))))
...
bool RV3028C7::setDailyAlarm(uint8_t hour, uint8_t minute) { return setDateAlarm(ALARM_ONCE_PER_DAY, 1, hour, minute); }

bool RV3028C7::setHourlyAlarm(uint8_t minute) { return setDateAlarm(ALARM_ONCE_PER_HOUR, 1, 0, minute); }

bool RV3028C7::disableAlarm() { return setDateAlarm(ALARM_DISABLED, 1, 0, 0); }

bool RV3028C7::setPeriodicCountdownTimer(uint16_t timerValue, TimerClockFrequency_t frequency, bool repeat)
{
	if (timerValue < 1)
	{
		return false;
	...
	uint8_t control1 = readByteFromRegister(REG_CONTROL_1);
	if (!writeByteToRegister(REG_CONTROL_1, (control1 & ~(1 << BP_REG_CONTROL_1_TE))))
	{
		return false;
	}
	uint8_t control2 = readByteFromRegister(REG_CONTROL_2);
	if (!writeByteToRegister(REG_CONTROL_2, (control2 & ~(1 << BP_REG_CONTROL_2_TIE))))
	{
		return false;
	}
...
uint32_t RV3028C7::convertToUnixTimestamp(uint16_t year, uint8_t month, uint8_t dayOfMonth, uint8_t hour, uint8_t minute, uint8_t second)
{
	struct tm tm;
	tm.tm_sec = second;
	tm.tm_min = minute;
...
uint8_t RV3028C7::convertToDecimal(uint8_t bcd) { return (bcd / 16 * 10) + (bcd % 16); }

uint8_t RV3028C7::convertToBCD(uint8_t decimal) { return (decimal / 10 * 16) + (decimal % 10); }
...
bool RV3028C7::readBytesFromRegisters(uint8_t startAddress, uint8_t *destination, uint8_t length)
{
...
bool RV3028C7::writeBytesToRegisters(uint8_t startAddress, uint8_t *values, uint8_t length)
{
	_i2cPort->beginTransmission(RV3028C7_ADDRESS);
	_i2cPort->write(startAddress);
	for (uint8_t i = 0; i < length; i++)
