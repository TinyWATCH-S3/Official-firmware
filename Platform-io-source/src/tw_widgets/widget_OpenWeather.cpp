...
void WidgetOpenWeather::draw(uint canvasid)
{
	if (millis() - next_update > settings.config.open_weather.poll_frequency || next_update == 0)
	{
		...
		if (!url.isEmpty())
		{
            wifi_controller.add_to_queue(url, [](bool success, const String& response) {
                ow_widget.process_weather_data(success, response);
            });				
		}
	}
...
bool WidgetOpenWeather::click_double(uint16_t click_pos_x, uint16_t click_pos_y)
{
	return false;
}

bool WidgetOpenWeather::click_long(uint16_t click_pos_x, uint16_t click_pos_y)
{
	return false;
}
