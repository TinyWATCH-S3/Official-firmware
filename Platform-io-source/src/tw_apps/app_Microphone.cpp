 ...
 * @param style_hint 
 */
void AppMicrophone::draw_icon(uint canvasid, uint _pos_x, uint _pos_y, uint8_t style_hint)
{
    if (!is_icon_cached)
	{
		is_icon_cached = true;
 ...
 * @return false 
 */
bool AppMicrophone::click(uint16_t click_pos_x, uint16_t click_pos_y)
{
	visual_state++;
	if (visual_state == 4)
		visual_state = 0;
	
	Serial.printf("visual state %d\n", visual_state);
	return true;
}
 ...
 * @return false 
 */
bool AppMicrophone::click_double(uint16_t click_pos_x, uint16_t click_pos_y)
{
	return false;
}
