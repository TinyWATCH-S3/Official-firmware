	...
	if (force || millis() - next_update > update_period)
	{
		...
		if (!is_dragging || !is_cached)
		{
			...
			for (int i = 0; i < 6; ++i) {
				...
				canvas[canvasid].drawString(String(digit_num[i]),
				x_offset + (posmul * box_positions[i]) + (x_space * box_positions[i]) + digit_xoffset,
...
bool FaceWatch_CustomBinary::click(uint pos_x, uint pos_y)
{
...
bool FaceWatch_CustomBinary::click_double(uint pos_x, uint pos_y)
{
...
bool FaceWatch_CustomBinary::click_long(uint pos_x, uint pos_y)
{
